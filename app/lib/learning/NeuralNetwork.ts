...
interface ModelWeights {
    weights_ih: number[][]; // Input -> Hidden
    weights_ho: number[][]; // Hidden -> Output
...
export class NeuralNetwork {
    ...
    constructor(input_nodes = FEATURE_COUNT, hidden_nodes = HIDDEN_NODES, output_nodes = OUTPUT_NODES) {
    ...
    async load(): Promise<void> {
        ...
        try {
            ...
            if (data && data.weights) {
                const w = data.weights as ModelWeights;

                // Reconstruct Matrices from JSON data
                this.weights_ih = new Matrix(w.weights_ih.length, w.weights_ih[0].length);
                this.weights_ih.data = w.weights_ih;

                this.weights_ho = new Matrix(w.weights_ho.length, w.weights_ho[0].length);
                this.weights_ho.data = w.weights_ho;

                this.bias_h = new Matrix(w.bias_h.length, w.bias_h[0].length);
                this.bias_h.data = w.bias_h;

                this.bias_o = new Matrix(w.bias_o.length, w.bias_o[0].length);
                this.bias_o.data = w.bias_o;

                this.generation = data.generation;
                this.isInitialized = true;
                logger.info(`[NeuralNetwork] Loaded generation ${this.generation}`);
            } else {
                logger.info('[NeuralNetwork] No saved model found, using random weights');
                await this.save(); // Save initial random weights
                this.isInitialized = true;
            }
        } catch (error) {
            logger.error('[NeuralNetwork] Failed to load weights', error);
    ...
    async save(): Promise<void> {
        try {
            ...
            logger.info(`[NeuralNetwork] Saved generation ${this.generation}`);
        } catch (error) {
            logger.error('[NeuralNetwork] Failed to save weights', error);
        }
    }
     ...
     */
    async train(input_array: number[], target_val: number): Promise<void> {
        ...
        const targets = Matrix.fromArray([target_val]);
        ...
        // Calculate gradient
        const gradients = Matrix.map(outputs, this.dsmoid);
        ...
        const hidden_gradient = Matrix.map(hidden, this.dsmoid);
        ...
        logger.info(`[NeuralNetwork] Trained on target ${target_val}. New Gen: ${this.generation}`);
    }

    // Helper for dsigmoid mapping
    private dsmoid = (y: number) => y * (1 - y);
}
